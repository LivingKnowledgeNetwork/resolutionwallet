...
casper.test.comment("Validate a new compute type (symbolic use case)");
...
casper.then(function() {
  ...
  this.mouseEvent('click', '#lkn-validate-compute');

});

casper.then(function() {
	this.test.comment('check the validation has passed');
	casper.test.assertExists('#lkn-validate-compute-status', 'the element exists');
  var textP = 'passed';
