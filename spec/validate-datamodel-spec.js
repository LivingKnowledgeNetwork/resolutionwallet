...
casper.test.comment("Validate a new datamodel");
...
casper.then(function() {
  ...
  this.mouseEvent('click', '#lkn-validate-datamodel');

});

casper.then(function() {
	this.test.comment('check the validation has passed');
	casper.test.assertExists('#lkn-validate-datamodel-status', 'the element exists');
  var textP = 'passed';
